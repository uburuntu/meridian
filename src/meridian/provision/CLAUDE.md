...
- **HTTP/2 via `http2 on;`** — nginx 1.25+ deprecated `listen ... http2`. Use `http2 on;` as a separate directive in the server block. Without it, ALPN only negotiates HTTP/1.1 — a fingerprinting vector since all modern servers support h2.
